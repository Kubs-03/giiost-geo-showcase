        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-6">
              {valueProps.map(prop => <div key={prop.id} className={`p-6 rounded-lg border-2 cursor-pointer transition-all duration-300 ${activeValue === prop.id ? 'border-primary bg-primary/5' : 'border-grey-200 hover:border-grey-300'}`} onMouseEnter={() => setActiveValue(prop.id)}>
                  <div className="flex items-start space-x-4">
                    <div className={`p-3 rounded-lg ${activeValue === prop.id ? 'bg-gradient-primary' : 'bg-grey-100'}`}>
                      <prop.icon className={`h-6 w-6 ${activeValue === prop.id ? 'text-white' : 'text-grey-600'}`} />
                    </div>
                    <div className="flex-1">
                      <h3 className={`text-xl font-semibold mb-2 ${activeValue === prop.id ? 'text-primary' : 'text-foreground'}`}>
                        {prop.title}
                      </h3>
